...
public class p1TylerJohnson extends PApplet {
...
public void thirty() {
  if(setupComplete) {
    timeS += 30;
    if(PApplet.parseInt(timeS / 60) != 0) {
      timeM += + PApplet.parseInt(timeS / 60);
    }
    timeS %= 60;
    timeM %= 100;
  }
  ...
  if(setupComplete && !running) {  
    ...
    if(!presetUsed) {
      if(tcMode) {
        ...
        timeM = timeM * 10 + PApplet.parseInt(timeS / 100);
        timeS %= 100;
        timeM %= 100;
      } else {
        timeM += value;
...
public void preset() {
  ...
  static public void main(String[] passedArgs) {
    String[] appletArgs = new String[] { "p1TylerJohnson" };
    if (passedArgs != null) {
